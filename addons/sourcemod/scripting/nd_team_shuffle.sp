...
void PullSpectators()
{
	...
	{
		if (IsValidClient(client) && !ND_IsCheckedAFK(client) && GetClientTeam(client) <= TEAM_SPEC)
		{
...
int GetTopTwoSkillDiff(bool roundStarted)
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		if (IsValidClient(client) && (!roundStarted || IsReadyForBalance(client, roundStarted)))
		{
