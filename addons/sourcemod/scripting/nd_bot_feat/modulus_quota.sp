...
{
	int maxTurretBots = g_cvar[turretBotDec].IntValue;
	if (ND_GetTurretCount() >= g_cvar[turretCountDec].IntValue && totalCount > maxTurretBots)
		totalCount = maxTurretBots;
		
	return g_cvar[BoosterQuota].IntValue;
}
