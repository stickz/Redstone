...
/*enum SWConvars
{
	ConVar:expLevelEighty,
	ConVar:expBaseMulitpler,
	ConVar:expGrowthInterval,
	ConVar:expDecayInterval,
	ConVar:expDecaySkill
};
ConVar gc_SteamWorks[SWConvars];

void SteamWorks_CreateConvars() 
{
	gc_SteamWorks[expLevelEighty]		= 	CreateConVar("sm_sw_eightyExp", "500000", "Expereince for player to be considered level 80");
	gc_SteamWorks[expGrowthInterval] 	=	CreateConVar("sm_sw_egi", "20", "Specifies the skill increase at each exponential tick");
	gc_SteamWorks[expDecayInterval]		=	CreateConVar("sm_sw_edi", "5", "Specifies the skill decrease at each exponential tick");
	gc_SteamWorks[expDecaySkill]		= 	CreateConVar("sm_sw_dc", "5000", "Specifies skill value between each decay tick");
	gc_SteamWorks[expBaseMulitpler]		=	CreateConVar("sm_sw_expMult", "1.95", "Specifies the multipler between each exponential tick");
}*/